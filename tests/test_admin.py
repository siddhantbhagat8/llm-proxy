...
def test_create_user_returns_api_key(client):
    ...
    assert created["limits"]["requests_per_minute"] == 5
    models = client.get(
...
def test_update_limits_sets_and_clears_independently(client):
    response = client.put(
        ...
        json={"requests_per_minute": 9, "lifetime_spend_dollars": None},
        headers=ADMIN_HEADERS,
    )
    assert response.status_code == 200
    limits = response.json()["limits"]
    assert limits["requests_per_minute"] == 9
    assert limits["lifetime_spend_dollars"] is None
    assert limits["tokens_per_day"] == 1_000  # untouched field unchanged
